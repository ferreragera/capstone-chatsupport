                        ...
                        <table class="table hover" id="archiveTable">
                            ...
                            <tbody>
                                @foreach($archive as $value)
                                    <tr>
                                        <th>{{ $value->tag }}</th>
                                        <td>{!! is_array($value->patterns) ? implode('<br>', $value->patterns) : $value->patterns !!}</td>
                                        <td>{!! is_array($value->responses) ? implode('<br>', $value->responses) : $value->responses !!}</td>
                                        <td>
...
@section('script')
    ...
    $(document).ready(function() {
        ...
        $('#archiveTable').on('click', '.back-btn', function() {
            ...
            var patterns = row.find('td:eq(0)').html().trim().split('<br>'); // Get patterns from the second column
            var responses = row.find('td:eq(1)').html().trim().split('<br>'); // Get responses from the third column
