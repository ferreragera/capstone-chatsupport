...
class Chatbox {
    ...
    onSuggestedTopicClick(topic, chatBox) {
        fetch('http://192.168.43.86:5000/predict', {
            method: 'POST',
    ...
    updateChatText(chatbox) {
        ...
        this.messages.slice().reverse().forEach(function (item, index) {
            if (item.name === 'CVSU Admission System') {
                ...
                if (item.clickable) {
                    // Introduce a 5-second delay before displaying jumping dots animation for operator messages
                    setTimeout(() => {
                        html += '<div class="messages__item messages__item--operator">';
                        html += '<span class="jumping-dots">';
                        html += '<span class="dot-1"></span>';
                        html += '<span class="dot-2"></span>';
                        html += '<span class="dot-3"></span>';
                        html += '</span>';
                        html += '</div>';
